...
#' 		\time{target_error}{root mean(devided by nxm) square error of the target function. Same as error if no penalty}
#' 	}
...
nnmf <- function(x, k, method = c('nnls', 'brunet'), eta = 0, beta = 0, max.iter = 500L, tol = 1e-5) {
	...
	out <- switch(method,
		'nnls' = .Call('nmf_nnls', x, as.integer(k), as.double(eta), as.double(beta), as.integer(max.iter), as.double(tol)),
		'brunet' = .Call('nmf_brunet', x, as.integer(k), as.integer(max.iter), as.double(tol))
		);
...
predict.NNMF <- function(object, newdata, which.matrix = c('H', 'W'), max.iter = 500L, tol = object$tol) {
	...
	switch(object$method,
		...
		'brunet' = {
			switch(which.matrix,
				'H' = .Call('get_H_brunet', newdata, object$W, max.iter, tol),
				'W' = t(.Call('get_H_brunet', t(object$H), t(newdata), max.iter, tol)))
			}
		)
	}
