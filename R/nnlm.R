...
#' 	\item option       : list of information of input arguments
...
nnlm <- function(x, y, alpha = rep(0, 3), method = c('scd', 'lee'),
	loss = c('mse', 'mkl'), mask = NULL, init = NULL, check.x = TRUE,
	max.iter = 10000L, rel.tol = .Machine$double.eps, n.threads = 1L) {
	...
	y <- check.input.matrix(y);
	x <- check.input.matrix(x, check.missing = TRUE);
	...
	beta <- c(beta, rep(0, 3))[1:3];
	if (show.warning && beta[1] < beta[2])
		warning("If beta[1] < beta[2], be aware that that algorithm may not converge or unique.");

	mask <- check.input.matrix(init, dim = c(ncol(x), ncol(y)), vars = list('x', 'y'), mode = 'logical');
	init <- check.input.init(init, dim = c(ncol(x), ncol(y)), vars = list('x', 'y'));
	# if masked but no initialized, masked entries are thought to fix to 0
	if (length(mask) != 0 && length(init) == 0)
		init <- as.numeric(!mask);
	...
	error <- mse.mkl(y, x %*% sol$coefficients, na.rm = TRUE, warning = FALSE);
	...
	target.error <- target.error + (beta[1] - beta[2])*sum(sol$coefficients^2) +
			beta[2]*sum(colSums(sol$coefficients)^2) + beta[3]*sum(sol$coefficients);

	sol$error <- c(error, 'target.error' = target.error);
	sol$option <- list('method' = method, 'loss' = loss, 'max.iter' = max.iter, 'rel.tol' = rel.tol);
	sol$call <- match.call();
