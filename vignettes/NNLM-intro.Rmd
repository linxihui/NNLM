...
Assume $S = {s_1, ..., s_L}$, where $s_l, \, l = 1, ..., L$ is a set of genes in sub-network $s_l$, then one can
...
Since matrix $A$ is assumed to have low rank $k$, information in $A$ is redundant for the decomposition,
thus it possible to allow some entries in $A$ absent.  One can just use the non-missing entries to compute
$W$ and $H$. Such methodology can be used to imputation the missing entries in $A$. This has an application
...
An recomendation can be done simply based on the predicted scores.  In additon, the resulting $W$ and $H$ 
